...
const Layout = ({ children }: IProps) => {
  ...
  const [activeItem, setActiveItem] = React.useState('Homepage');
  const handleSideBar = () => {
    setOpen((prev) => !prev);
  };
    ...
    <>
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        >
          ...
          <List>
            {[
              { label: 'Dashboard', to: '/admin', icon: RiHome4Line },
              { label: 'Homepage', to: '/admin/community', icon: RiGlobalLine },
            ].map((item) => (
            ...
            {[
              { label: 'Cohorts', to: '/admin/cohorts', icon: RiDashboardLine },
              { label: 'Members', to: '/admin/members', icon: RiGroupLine },
              { label: 'Saves', to: '/admin/saves', icon: BiBookmarks },
            ].map((item) => (
              <ListItem key={item.label} disablePadding sx={{ display: 'flex' }}>
                <ListItemButton
                  sx={{
